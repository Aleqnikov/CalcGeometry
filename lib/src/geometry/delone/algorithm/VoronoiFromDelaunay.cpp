...
static bool is_real_face(const DCEL& dcel, size_t fi) {
    if (fi == INVALID || dcel.is_infinite_face(fi))
        return false;

    if (dcel.face_edge(fi) == INVALID)
        return false;
...
static Point2D outward_direction(const DCEL& dcel, size_t e) {
    size_t orig = dcel.edge_origin(e);
    size_t dest = dcel.edge_origin(dcel.edge_twin(e));

    Point2D a = dcel.vertex_pos(orig);
    Point2D b = dcel.vertex_pos(dest);

    double dx = b.x - a.x, dy = b.y - a.y;
    double mx = (a.x+b.x)*0.5, my = (a.y+b.y)*0.5;

    size_t fi = dcel.edge_face(e);
    size_t v0, v1, v2;
    dcel.face_vertices(fi, v0, v1, v2);

    double cx = (dcel.vertex_pos(v0).x +
                 dcel.vertex_pos(v1).x +
                 dcel.vertex_pos(v2).x) / 3.0;

    double cy = (dcel.vertex_pos(v0).y +
                 dcel.vertex_pos(v1).y +
                 dcel.vertex_pos(v2).y) / 3.0;
...
VoronoiDiagram voronoi_from_delaunay(const DCEL& dcel) {
    ...
    for (size_t ei = 0; ei < dcel.edge_count(); ++ei) {
        size_t ti = dcel.edge_twin(ei);

        if (ti == INVALID)
            continue;

        if (!visited.insert(std::min(ei, ti)).second)
            continue;

        size_t fi = dcel.edge_face(ei);
        size_t fj = dcel.edge_face(ti);

        if (fi == INVALID || fj == INVALID)
            continue;
        ...
        if (ri && rj) {
            ...
            auto ci = circumcenter(dcel.vertex_pos(v0),
                                   dcel.vertex_pos(v1),
                                   dcel.vertex_pos(v2));

            dcel.face_vertices(fj, v0, v1, v2);

            auto cj = circumcenter(dcel.vertex_pos(v0),
                                   dcel.vertex_pos(v1),
                                   dcel.vertex_pos(v2));
            ...
            size_t v0, v1, v2;
            dcel.face_vertices(real_fi, v0, v1, v2);
